...
- **Laravel config and env key navigation.** "Go to Definition" and "Find All References" now work for string-literal config keys and env variables. Ctrl-clicking `config('app.name')`, `Config::get('app.name')`, or `Config::set('app.name')` jumps to the matching key declaration in `config/app.php`. Ctrl-clicking `env('APP_KEY')` jumps to the corresponding line in the highest-priority env file (`.env.local` › `.env` › `.env.testing` › `.env.production` › `.env.staging` › `.env.example`). "Find All References" on a config key lists every `config()`, `Config::get()`, and `Config::set()` call site plus the declaration in `config/*.php`. "Find All References" on `env('APP_KEY')` lists every PHP call site; with "Include Declaration" it also highlights the `APP_KEY=` line in every env file that defines the key. Cursoring on an `APP_KEY=` line inside a `.env*` file and invoking "Find All References" lists all PHP `env('APP_KEY')` usages across the project. Navigation is gated on the presence of an `artisan` file, so non-Laravel PHP projects (Symfony, etc.) are unaffected.
